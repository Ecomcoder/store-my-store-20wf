...
export function ReviewsWidget({ productId, storeEnvironmentId, publishableApiKey }: ReviewsWidgetProps) {
  const [page, setPage] = useState(1)
  const perPage = 5

  const { data, isLoading } = useReviews(productId, { page, perPage, storeEnvironmentId, publishableApiKey })
  const { mutate: createReview, isPending, isSuccess } = useCreateReview({ storeEnvironmentId, publishableApiKey })
